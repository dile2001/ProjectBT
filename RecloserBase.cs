    ...
    public class RecloserBase:INotifyPropertyChanged, ICommDevice
    {
        ...
        public bool Client
        {
            ...
            set
            {
                ...
                if (_client == true)
                {
                    _listener = new TcpClient(this, _serveraddress, _port);
                }
                else
                {
        ...
        public RecloserBase(int port)
        {
            ...
            this.timerSuspend.Interval = 20 * 60 * 1000; //suspend 15 minutes
                ...
                if (_request != null && _request.Count > 0)
                {
                    ...
                    try
                    {
                        if (!string.IsNullOrEmpty(this.StartPackage))
                        {
                            ...
                            if (s != string.Empty)
                            {
                                this.Listener.Send(TcpComm.Ultility.FromHex(s));
        ...
        protected bool CheckConnectionStatus()
        {
            ...
            if ((DateTime.Now - this.LastUpdated).TotalMilliseconds >= (timerBetweenEachPoll.Interval * 2<120000? 600000 : timerBetweenEachPoll.Interval * 2) && this.LastUpdated != DateTime.MinValue) 
            {
                this.timerSuspend.Enabled = true;
                pollingcount = 0;
                this.Stop();
                FA_Accounting.Common.LogService.WriteInfo("Reset connection " + this.Name + " at " + pollingcount.ToString(), "" );
                return false;
            }
            return true;
        }
